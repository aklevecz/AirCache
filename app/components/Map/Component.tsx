...
export const Map = forwardRef<Ref, Props>(({ initMap, map, user = undefined }, ref) => {
  ...
  useEffect(() => {
    loader.load().then(() => {
      if (mapContainer.current) {
        ...
        const center = last_location ? JSON.parse(last_location) : { lat: NY_COORDS.lat, lng: NY_COORDS.lng };
  ...
  return <div ref={mapContainer} style={{ height: "100%", width: "100%" }}></div>;
});
